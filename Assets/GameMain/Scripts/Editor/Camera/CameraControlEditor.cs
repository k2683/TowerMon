...
namespace TowerMon
{
    ...
    public class CameraRigEditor : UnityEditor.Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            var fmh_42_69_638695665967537875 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
                                                    Handles.RectangleHandleCap);
            var fmh_44_63_638695665967559450 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
                                                 Handles.RectangleHandleCap);
            var fmh_46_65_638695665967560920 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
                                                  Handles.RectangleHandleCap);
            var fmh_48_67_638695665967561846 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
                                                   Handles.RectangleHandleCap);
